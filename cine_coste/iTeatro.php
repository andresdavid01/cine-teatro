...
<html lang="en">
  <head>
    ...
    <script type="text/javascript">
      function elimina(id_teatro){
			$.ajax({
			    type: "POST",
			    url: "controllers/eTeatro.php",
			    data: "id_teatro="+id_teatro,
	...
	function edit(id_teatro, nombre_teatro,id_zona_teatro){
		document.getElementById("id_teatro").value=id_teatro;
		document.getElementById("nombre_teatro").value=nombre_teatro;
...
alert("aqui");
		$("#ingresar").click(function(){ 
			id_teatro=$("#id_teatro").val();
			nombre_teatro=$("#nombre_teatro").val();
			id_zona_teatro=$("#id_zona_teatro").val();

			 $.ajax({
			    type: "POST",
			    url: "controllers/iTeatro.php",
			    data: "id_teatro="+id_teatro+"&nombre_teatro="+nombre_teatro+"id_zona_teatro="+id_zona_teatro,
			    success: function(html){ 
alert(html+"info");
			    if(html=='1'){
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" class="form-control" id="id_teatro" placeholder="Código del teatro" required />
    </div>
  </div>
  <div class="form-group">
    <label for="inputPassword3" class="col-sm-2 control-label">Nombre</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="nombre_teatro" placeholder="nombre del teatro" required />
    </div>
	  <div class="form-group">
    <label for="inputPassword3" class="col-sm-2 control-label">Zona_teatro</label>
    <div class="col-sm-10">
      <select name="zona_teatro">
		<?php
		ini_set('display_errors', 'on');
		include_once("models/class.teatro.php");
		$obj = new teatro;
		$obj->lista_zona_teatro();
		?>
	  </select>
    </div>
  </div>
